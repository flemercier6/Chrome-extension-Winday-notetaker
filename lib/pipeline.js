...
export async function processLive(blob, meeting, transcript, opts) {
  ...
  const { payload, metadata } = liveMeetingRow(m, transcript, userId, audioError);
  await sb.insertMeeting(payload);
...
export async function importTranscript(meeting, transcript, opts) {
  ...
  onStage("uploading");
  const { payload, metadata } = liveMeetingRow(m, transcript, userId);
  payload.metadata.imported = true; // the row has no audio BY DESIGN, not by failure
  await sb.insertMeeting(payload);
  m.transcript = metadata.transcript;
...
async function ensureMeetingRow(m, utterances, userId) {
  ...
  if (!row) {
    ...
    if (utterances.length > 0) {
      await sb.insertMeeting(liveMeetingRow(m, m.transcript, userId).payload);
      return;
    }
    const stoppedAt = m.endedAt || new Date().toISOString();
    const payload = {
