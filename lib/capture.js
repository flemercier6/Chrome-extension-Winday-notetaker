...
export function createRecorder() {
  ...
  async function finalize() {
    ...
    try {
      ...
      // Deepgram: the live transcript is authoritative; the batch pass is only
      // a fallback when streaming produced nothing (WS blocked, no speech, …).
      // Gladia: the saved transcript ALWAYS comes from the batch pass — its
      // async pre-recorded mode has full diarization and the best accuracy,
      // while its live mode (display-only here) has neither.
      const useLive = provider() !== "gladia" && transcript.utterances.length > 0;
