 ...
 *  into the meeting's metadata. jsonb PATCH replaces the whole column, so the
 *  current metadata is read first and merged (RLS scopes both to the owner). */
export async function updateMeetingSummary(id, summary) {
  const token = await accessToken();
  ...
  const metadata = { ...(rows[0].metadata || {}), summary };
...
/** Renames a meeting (RLS: only the owner's row can match). */
export async function renameMeeting(id, title) {
  const token = await accessToken();
  const resp = await fetch(endpoint(`/rest/v1/meetings?id=eq.${encodeURIComponent(id)}`), {
    method: "PATCH",
    headers: { apikey: ANON, Authorization: `Bearer ${token}`, "Content-Type": "application/json", Prefer: "return=minimal" },
...
export async function listMeetings(limit = 50) {
  const token = await accessToken();
  const cols = "id,meeting_title,status,started_at,stopped_at,duration_seconds,metadata";
...
async function notionCall(action, method) {
  const token = await accessToken();
